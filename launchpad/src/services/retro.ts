...
import type { LaunchRow, OutcomeFormInput } from '../types';
 ...
 */
export async function postRetroPrompt(
  client: WebClient,
  launch: LaunchRow
): Promise<void> {
 ...
 */
export function buildOutcomeModal(launchId: number, launchName: string) {
  return {
    type: 'modal' as const,
    callback_id: 'retro_outcome_submit',
    private_metadata: String(launchId),
    title: { type: 'plain_text' as const, text: 'Launch Retro' },
    submit: { type: 'plain_text' as const, text: 'Submit & Archive' },
 ...
 */
export async function finalizeRetroAndArchive(
  client: WebClient,
