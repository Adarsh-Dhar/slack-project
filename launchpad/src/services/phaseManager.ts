...
import type { LaunchRow, LaunchPhase, TeamName } from '../types';

const PHASE_ORDER: LaunchPhase[] = ['discovery', 'build', 'prelaunch', 'gonogo', 'launchday'];

export function calculatePhase(launchDate: string): LaunchPhase {
...
async function resolveTeamMembers(
  ...
  if (roster?.manual_user_ids) {
    return JSON.parse(roster.manual_user_ids) as string[];
  }

  return [];
}

function findSubChannelForTeam(launchId: number, team: TeamName): string | null {
  const channels = db.getStakeholderChannels(launchId);
  const match = channels.find(c => c.team === team);
  return match?.channel_id ?? null;
}

export async function syncMembersForPhaseChange(
  client: WebClient,
  launch: LaunchRow,
  oldPhase: LaunchPhase,
  ...
  for (const team of teamsToAdd) {
    const channelId = findSubChannelForTeam(launch.id, team as TeamName);
    if (!channelId) continue;

    const userIds = await resolveTeamMembers(client, launch.id, team as TeamName);
...
export async function announcePhaseChange(
  client: WebClient,
  launch: LaunchRow,
  newPhase: LaunchPhase,
  added: string[],
...
export async function checkAndSyncPhase(
  client: WebClient,
  launch: LaunchRow
): Promise<void> {
